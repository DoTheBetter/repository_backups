...
2. **GitHub Hosts 更新**：自动获取 GitHub Hosts 以解决 DNS 污染导致的连接问题。
3. **下载优化**：通过 jsDelivr CDN、解析 IP 或反代镜像等多重链路，确保安装包下载成功率。
4. **智能配置**：
    - 自动检测 CPU 微架构并配置最优 `core_version`。
    - 强制将内核更新分支设为 Dev。
    - 自动根据剩余空间选择适配的 LightGBM 模型（Smart 内核模式下）。
5. **资源更新**：同步更新 GeoIP、GeoSite、大陆 IP 白名单等核心数据库及订阅配置。
...
[查看脚本源码](https://github.com/Aethersailor/Custom_OpenClash_Rules/blob/main/shell/install_openclash_dev_update.sh)

```bash
# 下载脚本
wget https://raw.githubusercontent.com/Aethersailor/Custom_OpenClash_Rules/main/shell/install_openclash_dev_update.sh

# 赋予执行权限并运行
chmod +x install_openclash_dev_update.sh
...
为了优化分流体验，本项目持续收集被错误分流的国内冷门域名。

符合上游要求的域名将被提交至 [felixonmars/dnsmasq-china-list](https://github.com/felixonmars/dnsmasq-china-list)，最终合并进 GeoSite 数据库（[Loyalsoldier/v2ray-rules-dat](https://github.com/Loyalsoldier/v2ray-rules-dat)），实现在 OpenClash 中绕过内核访问。
...
2. **分流异常**：未命中任何国内规则，被分流至"漏网之鱼"（Final）策略组。

建议同时满足以下条件：

1. **解析为中国 IP 地址**：域名解析结果为中国大陆 IP 地址。
...
在 GitHub Issues 或 Telegram 群组中反馈。
...
> 2. **上游检测**：维护者将校验域名是否符合上游收录标准（NS 服务器位于中国大陆）。
>     - **符合**：提交 PR 至 GeoSite 上游。被收录后，OpenClash 将自动绕过内核处理。
>     - **不符合**：永久保留在本项目的直连规则中，由国内 DNS 解析。
