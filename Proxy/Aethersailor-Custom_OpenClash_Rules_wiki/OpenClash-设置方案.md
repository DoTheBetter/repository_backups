...
首先设置运行模式，在页面下方点击切换到 Fake-IP 模式，然后上方的运行模式选择 `**Fake-IP（增强）**` ，`启用`使用 Meta 内核，然后点击页面下方的“保存配置”  
...
启用该功能后，所有包含在 GeoSite 数据库 CN 分类中的域名，都会绕过 Clash 内核进行访问。  
...
注意务必启用下方的`“禁止 Dnsmasq 缓存 DNS”`选项   
...
所有设置按照图中进行即可，务必启用 GeoIP Dat 数据库。  
...
如果你使用的是 Vless、hy2 等节点，建议使用品云的订阅后端，或者自行填写其他支持你的节点格式的第三方订阅后端（包括你自己搭建的后端）。  
...
|文件名|Custom_Clash.ini|Custom_Clash_Mainland.ini|Custom_Clash_Lite.ini|Custom_Clash_GFW.ini| 
|:-:|:-:|:-:|:-:|:-:|
|说明|全分组模板|反代规则模板|轻量化规则模板|gfwlist 规则模板|
|无 DNS 泄露|✅|✅|✅|✅|
|能否通过 DNS 泄露测试|✅|✅|✅|❌|
|基本国内外分流|✅|✅|✅|❌|
|gfwlist代理|✅|✅|✅|✅|
|流媒体分流|✅|✅|❌|❌|
|AI分流|✅|✅|❌|❌|
|海外网站处理方式|代理|代理|代理|直连|
|是否需要订阅后端可直连 Github|是|否|是|是|   
***
我应该选择使用哪个模板？  
* Custom_Clash.ini  
本项目的标准订阅转换模板，拥有最完善的分流规则，适配所有需求，推荐使用。  
无DNS泄露，可通过泄露检测网站的测试。  
使用本项目推荐机场请直接使用该模板。  
```
