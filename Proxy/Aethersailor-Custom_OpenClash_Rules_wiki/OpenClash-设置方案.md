...
1. MosDNS -> OpenClash  
结论：负优化  
MosDNS 基于 GeoSite/GeoIP 分流，Clash 内核的 DNS 模块本身已经具备类似的功能，徒增性能开销  

2. AdGuard Home -> OpenClash  
结论：适合有需要的人  
这种设置适合需要加载大量 DNS 去广告规则以及监控和记录内网 DNS 请求的用户  

3. OpenClash -> MosDNS  
结论：错误设置  
DNS 查询请求变成由路由本机直接发出，对于国内访问无影响但徒增性能开销，对于访问冷门域名以及访问海外域名的解析由直连海外 DNS 完成，获取到的基本都是最慢的 CDN 节点  

4. OpenClash -> SmartDNS 
结论：错误设置  
DNS 查询请求变成由路由本机直接发出，对于国内访问无影响但徒增性能开销，对于访问冷门域名以及访问海外域名的解析由直连海外 DNS 完成，获取到的基本都是最慢的 CDN 节点  

5. MosDNS -> OpenClash -> SmartDNS  
结论：脑残设置  
恩山大量此类脑残贴，作者不知 no-resolve 为何物，只能通过这种手段来阻止 DNS 泄露  
将各类优秀的工具组合起来实现最烂的效果，作者基本上是连基本的 DNS 工作原理都不清楚  

虽然 SmartDNS 可以设置由隧道走节点发出 DNS 解析请求，但网上100%的教程都没教用户设置这一步，最后导致的就是负优化  

6. OpenClash ->  AdGuardHome  
结论：脑残设置  
拦截 DNS 请求的功能，放在越靠前的位置，性能开销越小  
油管上甚至还有教程教小白在 OpenWrt 里安装 Docker 再安装俩 AdGuardHome，一个服务 Nameserver 一个服务 Fallback
这种真是极品啊，能把性能开销拉满并且集合所有坑点的教程，真好奇作者是咋想出来的  

其他五花八门的设置就不列举了，论坛写帖子的以及油管上做视频的，麻烦用点心吧  
