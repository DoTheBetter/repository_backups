...
首先希望小白们明白一点，不管是使用 OpenClash luci 界面设置+订阅转换生成配置，还是导入 .yaml 文件，本质上都是为内核创建一个符合用户需求、可供启动的配置文件。  

OpenClash 的各个设置特别是覆写设置，是作者将常见的配置文件参数的修改操作设置成了 luci 界面中的各个选项，由 OpenClash 在启动时对订阅转换服务生成的默认配置文件进行覆写修改。  

直接手工修改 yaml 文件确实有更高的自由度，一些更复杂的参数，以及订阅转换后端服务无法实现的一些策略组特性，确实必须通过修改 yaml 文件来实现。  

但是本项目实现的相关特性，均可使用 OpenClash 的界面使用既有选项完成，无需在配置文件中添加任何额外的参数。对于本项目来说，导入 yaml 文件没有任何意义。  
