...
* **本项目订阅转换模板不含任何大陆 IP 和域名的分流规则，不要试图在 OpenClash 以外的 Clash 软件中使用，非 OpenClash 环境下使用会引发严重的分流问题。**    
...
* **丰富的分流策略组。** 
包含流媒体服务、AI 工具、电商、游戏平台等在内的大量常见的分流策略组。 
...
> 不建议部署旁路由，强烈建议使用 OpenWrt 作为唯一主路由。  
...
软路由≠旁路由，我很反感网上那些一提软路由上来就是旁路由的教程，完全就是坑小白，为了搞旁路由而创造需求，刻意将网络环境复杂化。尤其某些看起来高大上复杂到像蜘蛛网一样的“家用网络”拓扑图，当笑话看就好，千万别模仿。  
...
网上某些教程教小白用 AdGuardHome 等插件一顿折腾，添加一堆没用的第三方 DNS，套娃一层套一层，然后弄出来个 5ms 甚至 10ms 更高的解析延迟，再告诉你这是“优化”了，这种纯属扯淡，不信你去用最垃圾的硬路由， DHCP 直接默认就给你分配运营商 DNS 的那种，你会发现网页打开就是光速，你猜为什么。

还有别迷信什么乐观缓存，Fake-IP 模式下搭配其他前置缓存插件只会导致更多的问题，路由器上什么缓存都没有浏览器自身的 DNS 缓存作用大。  

按照本项目的方案设置后，运行商 DNS 只用于解析中国大陆域名，不存在污染问题，完全不用担心运营商 DNS 没有 DoT/DoH 加密。海外域名全部远端解析取得离机场最近的 CDN，更没有污染。  
...
首选确保你的 WAN 口设置中勾选了“自动获取 DNS 服务器”，这样才能获取到运营商下发的 IPv4 DNS  
...
首先设置运行模式，在页面下方点击切换到 Fake-IP 模式，然后上方的运行模式选择 **Fake-IP（增强）** ，勾选使用 Meta 内核，然后点击页面下方的“保存配置”  

如果你的页面上没有“使用 Meta 内核”的选项，是正常的，因为后来 OpenClash 只有 Meta 一个内核可选。  

Fake-IP（增强）模式可以提供最佳的性能，如果出现了 NAT 问题，可以尝试切换为 Fake-IP（混合）模式。记得要勾选 UDP 转发，如果你的固件包含了 Docker 功能，直接选择 Fake-IP（TUN）模式即可  
...
OpenClash 一些兜底的分流数据库，必须更新，否则会对绕过大陆功能产生影响。按照图中设置即可，具体用途不多做解释，可以自行查找相关资料。  
...
**建议**选择 dev 版本，然后点击下方的一键更新。在 OpenClash 的“运行日志”页面可以查看更新结果，此操作可以顺带验证你的 OpenWrt 是否能顺利访问 Github 或者你在之前设置的 CDN 比如 testingcf  

如果使用过程中遇到错误，可以选择 master 版本，再点击一键更新即可切换回稳定版，目前来看一些新特性必须 dev 版本的 OpenClash 和 Meta 才能支持，所以非特殊情况强烈建议使用 dev 版本  

2024年12月18日：目前 master 版本和 dev 版本没有太大区别，自行选择即可  
...
PS：本项目维护者日常使用的也是 dev 版本的 OpenClash 和 Meta 内核  
...
OpenWrt 是主路由的情况下，此处勾选按照图中进行勾选。  

若要使用运营商通告的 DNS，勾选“追加上游 DNS”。  
...
> **页面下方有 NameServer、Fallback 和 Default-NameServer 三个服务器分组，在本方案中应当取消全部的服务器勾选。**
> **如果你坚持不愿意使用运营商提供的 DNS**，则不要勾选“追加上游 DNS”，并且在 NameServer 中保留第一个勾选，填入你要使用的国内第三方 DNS 的地址，例如 223.5.5.5。  
...
下方的“自定义规则”勾选后会出现文本框，可以添加你想附加的规则，规则相当丰富，具体格式见文本框内的注释，此处不做赘述。  
...
> 第三方订阅转换服务有泄露订阅链接的风险，且订阅转换服务异常会引起转换后的配置文件无法启动。  

> 因此强烈建议自建订阅转换服务！既可以保证隐私安全，又可确保其稳定性从而保证 OpenClash 每日更新后正常启动，这是进阶内容，小白可以放弃。  

> 注意：自建订阅服务请务必保证订阅转换后端所在的机器可以正常访问 Github 才能拉取订阅模板中的规则碎片，建议使用海外 VPS 进行搭建，若使用国内机器，建议搭配 Github520 加速规则确保机器可以正常访问 Github。  
...
如果你使用的是 Vless、hy2等节点，建议使用品云的订阅后端，或者自行填写其他支持你的节点格式的第三方订阅后端（包括你自己搭建的后端）  

本项项目目前提供两个订阅转换模板，内容相同，

| 文件名 | 说明 | 是否需要订阅后端可直连 Github |
|:-:|:-:|:-:|
| Custom_Clash.ini | 常规订阅转换模板 | ✅ |
| Custom_Clash_Mainland.ini | 反代订阅转换模板 | ❌ |  

注意订阅转换模板选择“自定义模板”然后在下方填入本项目的自定义模板地址：  
