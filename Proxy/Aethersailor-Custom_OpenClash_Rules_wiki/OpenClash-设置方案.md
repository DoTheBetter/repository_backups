...
按照图中设置进行勾选设置，务必勾选`绕过大陆`功能来提升访问和下载性能。  
...
OpenClash 一些兜底的分流名单，必须更新。按照图中设置即可，具体用途不多做解释，可以自行查找相关资料。  
...
首先，启用`“自定义上游 DNS 服务器”`，取消下方的 `Fallback` 和 `Default-NameServer` 下的所有勾选。  
...
2. `旁路由`或使用了 `SmartDNS` 之类的插件：  

取消勾选`“追加上游 DNS”`，在 `NameServer` 中保留第一个勾选，并将地址修改为运营商 DNS 地址，如果是使用 `SmartDNS` 则填入` SmartDNS 的地址或者本机地址`，以及`端口`。同时取消 `Fallback` 和 `Default-NameServer` 下的所有勾选。  

注意追加上游 DNS 和下方的 NameServer 只需要`二选一`即可。OpenWrt 是主路由的情况下，此处勾选按照图中进行勾选。  
