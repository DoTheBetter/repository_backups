...
同样，国内所有的第三方 DNS，均存在对海外域名的污染情况（因为都是递归 DNS 而上游存在污染），与运营商 DNS 一致。所谓的使用国内第三方 DNS 对抗污染纯初以讹传讹。    
...
网络上现存的“套娃”设置教程基本就是两种，一种是在 OpenClash 上游 DNS 设置其他的 DNS 插件，另一种是在 dnsmasq 后设置设置其他的 DNS 插件，再转发给 OpenClash  
...
首先，启用`“自定义上游 DNS 服务器”`，然后根据以下三种情况，选择你对应的使用环境进行设置：  
