...
本项目维护者在原版 adblockfilters 的分支上维护的增强版，在原版规则的基础上，增加了 anti-ad、OISD、PeterLowe 等流行的规则作为上游，保留了原项目的去重和去无效域名机制，大量规则有效治疗规则焦虑症。  

注意：以上规则有可能导致网页打开变慢，此类副作用和设备性能有关。  
...
**注意：如果使用广告拦截功能后，访问网页有明显变慢，说明设备性能不够，请弃用该功能。**  
...
```bash
curl -sSL https://gh-proxy.com/https://raw.githubusercontent.com/Aethersailor/Custom_OpenClash_Rules/refs/heads/main/shell/edit_custom_firewall_rules.sh -o edit_custom_firewall_rules.sh && chmod +x edit_custom_firewall_rules.sh && ./edit_custom_firewall_rules.sh
...
(
    ...
    curl -sS -4 -L --retry 5 --retry-delay 1 "https://gh-proxy.com/https://raw.githubusercontent.com/Aethersailor/adblockfilters-modified/refs/heads/main/rules/adblockdnsmasq.txt" -o "$TARGET_DIR/adblockfilters-modified-for-dnsmasq.conf" >/dev/null 2>/tmp/adblockfilters-modified-curl.log
    CURL_EXIT=$?
...
(
    ...
    curl -sSL -4 --retry 5 --retry-delay 1 https://gh-proxy.com/https://raw.githubusercontent.com/TG-Twilight/AWAvenue-Ads-Rule/main/Filters/AWAvenue-Ads-Rule-hosts.txt | \
