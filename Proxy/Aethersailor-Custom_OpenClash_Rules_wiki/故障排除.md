...
> 注意：SNAPSHOT 版本的 OpenWrt/ImmortalWrt 均不包含 luci，新装系统是无法进入 web 管理页面的，需要使用 SSH 完成配置文件修改，正常联网后再自行安装 luci。
...
- **LEDE 固件**：过往版本与 OpenClash 的“绕过大陆”功能存在过兼容性问题，当前版本未知，请自行尝试。  
...
> **旁路由是一种错误的设置方法，强烈建议使用主路由环境，抛弃旁路由。**
...
如果国内常见的主流网站出现分流不正常。说明你的 OpenClash 工作不正常，请先更新 GEOIP 数据库、GeoSite 数据库和大陆白名单，必要时请还原设置重新配置。  
