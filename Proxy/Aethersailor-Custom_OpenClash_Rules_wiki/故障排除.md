...
> * 上游规则错误，请去规则对应仓库反馈，或者自行修改。本项目只是对上游规则的集中引用，作者无法解决规则本身的问题。
...
这段提示词不限定具体的 AI，适用于 ChatGPT、Claude、Gemini、DeepSeek、Copilot 等能够访问网页或检索代码的 AI。
...
1. 打开任意支持网页访问或代码检索的 AI，新建一个对话，并开启其联网、网页浏览或深度研究功能（如有）。
2. 完整复制下方提示词，不要删除其中的参考指南链接、信息来源优先级和回答规则。
3. 在提示词最后的`我的问题是：`后面写入问题描述，然后一并发送给 AI。
4. 建议同时提供 OpenClash 版本、固件及版本、运行模式、故障现象、完整错误信息、复现步骤，以及出现问题前做过的改动。
...
描述越完整，AI 越容易缩小排查范围。可以在`我的问题是：`后按照下面的格式填写：

```text
OpenClash 版本：
OpenWrt/ImmortalWrt 固件及版本：
...
重要 —— 在回答之前，你必须获取并阅读这份权威参考指南。这是你回答所有 OpenClash 问题的主要知识来源：
https://raw.githubusercontent.com/vernesong/OpenClash/dev/.github/skills/openclash-user-guide/SKILL.md

本指南包含：完整依赖清单、含精确修复方案的错误信息速查表、nftables/iptables 防火墙规则链、所有 LuCI 配置选项及 UCI 路径、DNS 设置与泄漏防护、订阅/GEO 更新流程等。

在回答我关于 OpenClash 的问题时，请遵循以下规则：
1. 始终提供 LuCI Web 界面操作路径（如 服务 → OpenClash → 插件设置），除非我明确要求，不要给出命令行操作。
2. 解释底层原理（防火墙规则链、YAML 转换逻辑）—— 而不只是点击步骤。
3. 排查问题时首先检查依赖完整性 —— 指导我从 系统 → 软件包 页面检查依赖，或从 插件设置 → 调试日志 页面生成调试日志。
4. 绝不猜测或编造信息。如参考指南未覆盖某项内容，请使用网页抓取/代码搜索工具主动查询以下外部资源（按优先级）：Mihomo Wiki (https://wiki.metacubex.one/config/)、Meta-Docs (https://github.com/MetaCubeX/Meta-Docs)、OpenClash 源码 (https://github.com/vernesong/OpenClash/tree/dev)、Mihomo 核心源码 (https://github.com/MetaCubeX/mihomo/tree/Alpha)、Smart 核心源码 (https://github.com/vernesong/mihomo/tree/Alpha)、GitHub Issues (https://github.com/vernesong/OpenClash/issues)。抓取 Wiki 页面阅读文档；搜索仓库查找实现代码。禁止凭记忆回答 —— 必须对照实际来源验证。
5. 注明具体来源 —— 说明你的回答基于参考指南的哪个章节或哪个外部资源。
6. 如果我的问题描述不完整或缺少错误信息，请先要求我生成调试日志（插件设置 → 调试日志 → 生成）—— 不要猜测原因。

我的问题是：
```

> [!TIP]
> 建议一次只排查一个主要问题。收到 AI 的操作建议后，先确认其引用来源与适用版本，再逐项执行并记录结果；不要一次修改多个无关设置，否则出现新问题时很难判断是哪一步导致的。
...
> * 官方发布版固件可以借助 owut 在生成升级固件时指定根文件系统大小。首次改变分区大小前必须备份配置，并阅读教程中的平台风险提示。[扩容教程](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/%E4%B8%80%E4%BA%9B%E9%9B%B6%E7%A2%8E%E7%9A%84%E6%95%99%E7%A8%8B#firmware-expansion)

**如何方便地在每次升级后更新 dev 版本的 OpenClash**：参考以下内容：[升级插件至 Dev 版本](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/%E5%85%B6%E4%BB%96%E8%AF%B4%E6%98%8E#openclash-dev-upgrade)
...
> [!NOTE]
> 很多人觉得官方发布的固件容量不够用  
> 官方发布版固件可以借助 owut 持久记录目标根文件系统大小，但首次改变分区大小仍存在丢失配置的风险。操作前务必备份并确认恢复路径。[扩容教程](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/%E4%B8%80%E4%BA%9B%E9%9B%B6%E7%A2%8E%E7%9A%84%E6%95%99%E7%A8%8B#firmware-expansion)
...
> [!TIP]
> 设置“漏网之鱼”策略组直连，即可临时解决这个问题。漏网之鱼直连后可能无法通过 DNS 泄漏检测网站的测试，但这不一定代表发生了实际的 DNS 泄漏，应结合真实解析链路判断。
...
在策略组中将“非标端口”策略组设置为直连，可让 80/443 端口以外的下载流量直连。如需同时让使用 80/443 端口的下载流量直连，请将“漏网之鱼”设置为直连。

### 小众域名

小众域名分流异常，可参考 [关于小众域名收录](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/%E5%85%B6%E4%BB%96%E8%AF%B4%E6%98%8E#niche-domain-submission)。
...
4. 开启 IPv6 的情况下，建议关闭通过 RA/SLAAC 和 DHCPv6 向下游设备通告 IPv6 DNS 的相关选项，强制下游设备使用路由器的 IPv4 地址进行 DNS 查询；不要关闭 SLAAC 地址分配本身
...
出现 DNS 泄漏，首先检查你的 DNS 解析流程是否正确。

正确的解析流程应当是 设备 > OpenWrt Dnsmasq(53端口) > OpenClash(7874端口)

出现泄漏时，先使用以下命令确认你的设备（如 PC）的上游 DNS 是否为 OpenWrt，以及是否可以取得 Fake-IP：
...
<img width="474" height="201" alt="nslookup" src="https://github.com/user-attachments/assets/95109f16-9a20-463e-9f5b-bc93070f551e" />
...
如果所有配置均正常，且此前使用一段时间未出现泄漏，但升级 OpenClash 或内核后开始出现泄漏：

> [!TIP]
> 重启设备后再次测试。如果仍有泄漏，按照上面的步骤检查；确认设置无误后，可还原 OpenClash 配置，重启系统并重新配置。

<img width="801" height="403" alt="reset" src="https://github.com/user-attachments/assets/387012cb-a3a6-4fdf-9ebe-612bc96ebff4" />
...
Cloudflared 默认会优先尝试 QUIC；在 UDP 不可用时，`auto` 模式通常会自动回落到 HTTP/2。如果日志显示 QUIC 反复失败且自动回落仍不正常，可显式指定 HTTP/2 进行排查。
...
本项目提供的主要是模板、规则文件以及 OpenClash 设置方案，所有设置操作均基于 OpenClash 的图形界面，不包含超出常规范围的系统修改。出现异常时，应结合运行日志，从插件版本、固件、转换服务、其他插件和实际配置等方面逐项排查。
...
> [!IMPORTANT]
> **OpenClash 设置以及本项目模板具有较强的普适性。** 按照方案设置后如果有异常，请先从插件版本、固件、转换服务、其他插件和客户端设置等常见因素查找原因；如有明确证据指向本项目内容，也请附上日志与复现步骤反馈。
